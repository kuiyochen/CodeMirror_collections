...
(function(mod) {
  ...
  cmds.get_list_at_this_cursor = function(cm) {
    cm.operation(function() {
      var rangeCount = cm.listSelections().length;
      for (var i = 0; i < rangeCount; i++) {
        var range = cm.listSelections()[i];
        var output_str = "";
        var start_number = 0, end_number = 20;
        var max_limit = 500;
        var arr = get_two_numbers(cm.getRange(range.from(), range.to()));
        if (Number.isInteger(Number(arr[0])) && arr[0] !== ""){
          if (Number.isInteger(Number(arr[1])) && arr[1] !== ""){
            start_number = arr[0];
            end_number = arr[1];
          }else{
            end_number = arr[0];
        ...
        cm.replaceRange(output_str, range.from(), range.to());
      }
      // cm.scrollIntoView();
    });
  };

  cmds.get_index_1_by_1 = function(cm) {
    cm.operation(function() {
      var start_number = 0;
      var difference = 1;
      var range = cm.listSelections()[0];
  ...
  function get_two_numbers(str){
    ...
    if (arr.length == 2){
      if (Number.isInteger(Number(arr[0])) && arr[0] !== "" && Number.isInteger(Number(arr[1])) && arr[1] !== ""){
        first = Number(arr[0]);
        second = Number(arr[1]);
      }
      if (arr[1] == ""){
  ...
  cmds.bracket_changing_0 = function(cm) {bracket_changing(cm, ["(", ")"])}
  cmds.bracket_changing_1 = function(cm) {bracket_changing(cm, ["[", "]"])}
